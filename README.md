...
- **Timer:** Dauer ist einstellbar (Standard 20s, per Zahlenfeld änderbar) und über **"Timer starten"** auslösbar. Während der Timer läuft, kannst du ihn mit **"-5s"** live verkürzen oder mit **"Timer stoppen"** sofort beenden — die Anzeige im Overlay reagiert sofort mit.
- **Herzen-Modus:** In der Design-Box oben zwischen **2 Herzen** (hart) und **3 Herzen** (Standard) für das ganze Spiel umschaltbar. Setzt beim Wechsel die Herzen aller Spieler auf den neuen Wert zurück (mit Sicherheitsabfrage).
