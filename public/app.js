...
function defaultState(n){
  ...
  return { count:n, players, round:1, turnsThisRound:0, bgColor:'#1a1533', logoDataUrl:'', maxHearts: HEARTS_PER_PLAYER, timerDuration:20 };
}
...
function applyServerState(payload){
  ...
  if(payload.maxHearts === undefined) payload.maxHearts = HEARTS_PER_PLAYER;
  if(payload.timerDuration === undefined) payload.timerDuration = 20;
  currentState = payload;
}

/* Herzen-Modus für das ganze Spiel umstellen (z.B. 2-Herzen-Modus) - setzt alle Spieler auf den neuen Max-Wert zurück */
function setMaxHearts(state, n){
  state.maxHearts = n;
  state.players.forEach(p=>{ p.hearts = n; });
  return state;
}
...
/* ---------- Fragetimer (Dauer einstellbar) ---------- */
function startTimer(state, durationSec){
  const d = durationSec || state.timerDuration || 20;
  state.timerDuration = d;
  state.timer = { running:true, startedAt: Date.now(), duration: d*1000 };
  return state;
}
