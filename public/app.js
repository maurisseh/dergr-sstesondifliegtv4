...
function buildVdoViewSrc(input){
  ...
  // Zielt direkt auf eine ordentliche Bitrate, statt sich langsam hochzutasten
  if(!/[?&](videobitrate|vb)=/.test(url)){
    url += '&videobitrate=2500';
  }
  return url;
}
...
function buildVdoPushLink(input){
  ...
  // &quality=0 lässt die sendende Seite direkt 1080p anstreben statt sich hochzutasten
  return `https://vdo.ninja/?push=${encodeURIComponent(id)}&webcam&quality=0`;
}
